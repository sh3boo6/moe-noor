...
const stageLabels = ['رياض الأطفال', 'التعلىم المستمر', 'الابتدائية', 'المتوسطة', 'الثانوية']
...
const stageFilters = [
  ...
  (stage: string) => {
    const s = normalizeStage(stage)
    return s.includes('التعلىم المستمر')
  },
  (stage: string) => {
    const s = normalizeStage(stage)
...
const continuingEducationLabels = ['صف 1', 'صف 2', 'صف 3']

const stageGradeLabels = [
  kindergartenLabels,
  continuingEducationLabels,
  primaryGradeLabels,
  middleGradeLabels,
  highGradeLabels
]
...
const kindergartenGradeSeries = computed<ApexAxisChartSeries>(() => buildGradeGenderSeries(0))
const continuingEducationGradeSeries = computed<ApexAxisChartSeries>(() => buildGradeGenderSeries(1))
const primaryGradeSeries = computed<ApexAxisChartSeries>(() => buildGradeGenderSeries(2))
const middleGradeSeries = computed<ApexAxisChartSeries>(() => buildGradeGenderSeries(3))
const highGradeSeries = computed<ApexAxisChartSeries>(() => buildGradeGenderSeries(4))
...
const kindergartenChart = computed(() => gradeChartBaseOptions(kindergartenLabels, 'الصفوف الدراسية - رياض الأطفال', kindergartenGradeSeries.value))
const continuingEducationChart = computed(() => gradeChartBaseOptions(continuingEducationLabels, 'الصفوف الدراسية - التعليم المستمر', continuingEducationGradeSeries.value))
...
const genderDistribution = computed(() => {
  ...
  const boys = grouped.get('بنين') || 0
  const girls = grouped.get('بنات') || 0
  const other = grouped.get('غير محدد') || 0
  const result: Array<{ label: string, count: number }> = []
  if (boys > 0) result.push({ label: 'بنين', count: boys })
  if (girls > 0) result.push({ label: 'بنات', count: girls })
  if (other > 0) result.push({ label: 'غير محدد', count: other })
  return result
})
...
const continuingEducationAuthorityStageGender = computed(() => buildAuthorityStageGenderSeries(stageFilters[4]!))
...
const continuingEducationAuthorityStageGenderTotal = computed(() => {
  const data = continuingEducationAuthorityStageGender.value
  return getSeriesTotal(data.series)
})

const continuingEducationAuthorityStageGenderChart = computed<ApexOptions>(() =>
  authorityStageGenderChartOptions(continuingEducationAuthorityStageGender.value.authorities, continuingEducationAuthorityStageGender.value.series))
    ...
    <div class="grid gap-6 xl:grid-cols-2 xl:[&>*:last-child]:col-span-2">
      ...
      <UCard>
        <template #header>
          <div class="flex items-center justify-between gap-3">
            <h2 class="text-lg font-semibold text-foreground">
              توزيع المدارس حسب السلطة - التعليم المستمر - بنين / بنات ({{ formatNumber(continuingEducationAuthorityStageGenderTotal) }})
            </h2>
            <UIcon
              name="i-lucide-layers-3"
              class="h-5 w-5 text-primary"
        ...
        <ApexChart
          :key="chartKey"
          type="bar"
          :options="continuingEducationAuthorityStageGenderChart"
          :series="continuingEducationAuthorityStageGender.series"
          height="320"
        />
      </UCard>
    ...
    <div class="grid gap-6 xl:grid-cols-2 xl:[&>*:last-child]:col-span-2">
      <UCard>
        ...
        <ApexChart
          ...
          :series="kindergartenGradeSeries"
          height="300"
        />
      </UCard>
      ...
      <UCard>
        <template #header>
          <div class="flex items-center justify-between gap-3">
            <h2 class="text-lg font-semibold text-foreground">
              توزيع الطلاب حسب الصفوف - التعليم المستمر
            </h2>
            <UIcon
              name="i-lucide-chart-column"
              class="h-5 w-5 text-primary"
        ...
        <ApexChart
          :key="chartKey"
          type="bar"
          :options="continuingEducationChart"
          :series="continuingEducationGradeSeries"
          height="300"
        />
      </UCard>
      ...
      <UCard>
        <template #header>
          <div class="flex items-center justify-between gap-3">
            <h2 class="text-lg font-semibold text-foreground">
              توزيع الطلاب حسب الصفوف - الابتدائية
            </h2>
            <UIcon
              name="i-lucide-chart-column"
              class="h-5 w-5 text-primary"
      ...
      <UCard>
        <template #header>
          <div class="flex items-center justify-between gap-3">
            <h2 class="text-lg font-semibold text-foreground">
              توزيع الطلاب حسب الصفوف - المتوسطة
            </h2>
            <UIcon
              name="i-lucide-chart-column"
              class="h-5 w-5 text-primary"
      ...
      <UCard>
        <template #header>
          <div class="flex items-center justify-between gap-3">
            <h2 class="text-lg font-semibold text-foreground">
              توزيع الطلاب حسب الصفوف - الثانوية
            </h2>
            <UIcon
              name="i-lucide-chart-column"
              class="h-5 w-5 text-primary"
