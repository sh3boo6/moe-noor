...
<template>
  <UApp :toaster="toaster">
    ...
    <UModal v-model:open="isAboutOpen" title="حول التطبيق" icon="i-lucide-info"
      ...
      <template #body>
        <div class="space-y-4 text-right leading-7">
          <p>
            هذا التطبيق يساعدك على قراءة ملفات Excel الوزارية الشاملة وتحليل بيانات المدارس بسرعة، مع عرض مؤشرات إدارية
            ورسوم بيانية وجداول تفاعلية تدعم البحث والفلترة والتصدير.
          </p>
          <p>
            تم بناؤه كواجهة ويب و سطح مكتب ليكون أداة محلية مستقرة وسهلة الاستخدام لإدارة ومتابعة بيانات المدارس الصادرة
            من برنامج نور.
          </p>
    ...
    <UFooter :ui="{
      ...
      <template #right>
        <!-- <ClientOnly>
          <div v-if="!isDesktop" class="hidden lg:flex items-center gap-2">
            <UButton v-for="os in downloads" :key="os.os" :href="(os.links[0] as DownloadLink).href" variant="outline"
              color="neutral" size="sm" :icon="os.icon">
              {{ os.os }}
            </UButton>
          </div>
        </ClientOnly> -->
